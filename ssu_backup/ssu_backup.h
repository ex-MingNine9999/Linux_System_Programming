...
#define ROOT_HOME 2			// mac기준 root에서 home까지 / 3개: ubutu 2개
#define B_BACKUP 1
#define B_RECOVER 2
...
typedef struct _node {
	...
	pthread_t tid;					// 이 노드가 할당 받을 쓰레드 아이디
	struct _node *prev;
	struct _node *next;
} node;
...
int logCntl(int mode, char *msg);		// log파일 컨트롤 (mutex)
...
int backupFile(char *path, char *newPath, const int mode, dList *list);	// 주어진 파일에 대해 backup 실행
void optionD(const char *dname, char *com);	// add에 -d옵션 처리
void catFile(const char *path);			// 파일 내용 출력 cat
void *thread_backup(void *arg);			// 백업 쓰레드 함수
