				...
				if (buf[i] == '(') {
					...
					switch (k) {                                        // 라이브러리 함수가 등장했을때
						...
						case 3 :
							...
							sprintf(buf, "%s(%s, %s, %d);", cFunc[k], var, tbuf + l, sl);
...
int strInsert(char *buf, const char *in, int mode)
{
	...
	switch (mode) {
		case 1 :
			sprintf(buf, "%c->%s", in[0], tbuf);                // 구조체의 포인터에서 내부 변수를 참조하기 위해 변환
			break;
		case 2 :
			if (tbuf[0] == ')') {
				sprintf(buf, "%s *%c%s", in, in[0], tbuf);      // 구조체를 함수의 인자로 사용하기위해 인자로 삽입
			}
			else {
				sprintf(buf, "%s *%c, %s", in, in[0], tbuf);    // 인자로 넣을때 이미 인자가 존재할 경우
			}
			break;
		case 3 :
			sprintf(buf, "%s new%s", in, tbuf);                 // 생성자를 함수화
			break;
		default :
...
void insertFile(int fd, char *buf)
{
	...
	len = strlen(buf);                          // 삽입해야할 문자열의 길이만큼

	off = lseek(fd, -1, SEEK_END);              // 파일의 모든 문자를 뒤로 밀어 삽입
...
void optionProcessing(void)
{
	...
	if (optFlag[OPT_J]) {                       // 자바파일을 모두 fgets()로 한 줄씩 읽어 출력
	...
	if (optFlag[OPT_C]) {                       // 모든 c파일을 모두 fgets()로 한 줄씩 읽어 출력
	...
	if (optFlag[OPT_F]) {                       // 모든 파일을 lseek으로 크기를 확인하여 출력
		off_t jsiz, csiz;
	...
	if (optFlag[OPT_L]) {                               // 모든 파일을 fgets()로 더 이상 읽을 수 없을 때 까지 읽어 읽은 횟수로 라인 수 출력
		javaFp = fopen(filename, "r");
