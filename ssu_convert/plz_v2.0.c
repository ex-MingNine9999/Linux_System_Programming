...
int NumOfDef;
int optFlag[OPT_MAX];
char classVar[OBJ_MAX][VARLEN];
int NumOfVar;
char fdTable[OBJ_MAX][VARLEN];
int NumOfFd;
...
void setHeaderTable(void)
{
	...
	if ((fd = open(HEADER_TABLE, O_RDONLY)) < 0) {
...
{
	memset(funcMapping, -1, sizeof(funcMapping));
...
void convert()
{
	...
	char c;
	int i, j, len, ret = 0;
	int cb = 0, scb = -1, sck = 0, scbr = 0;
	int checkKey = 0, checkCb = 0;
	int javaLine = 0, cLine = 0;

	if ((javaFp = fopen(filename, "r")) == NULL) {
...
int convertLine(char oneLineBuf[BUFLEN], int cb)
{
	...
	if (cb == 0) {										// 함수의 외부
		...
		else if (strncmp(buf, "static void main", 16) == 0) {
		...
		if ((cp = strstr(buf, javaFunc[FUNC_NEW])) != NULL) {
			char *tp = cp;
			cp += 3;
			while (*(++cp) == ' ');
			...
			for (i = 0; i < C_DATA; i++) {
				if (strncmp(cp, DataType[i], strlen(DataType[i])) == 0) {
					...
					sprintf(tp, "(%s *)calloc(%s, sizeof(%s));",	DataType[i], tbuf, DataType[i]);
		...
		for (i = 0; i < NumOfVar; i++) {
			cp = buf;
			while ((cp = strstr(cp, classVar[i])) != NULL) {
				cp += strInsert(cp, cFile[NumOfcFile - 1], 1) + strlen(classVar[i]);
			}
		}
	}

	if (checkMain > 0 && strncmp(buf, "return", 6) == 0) {
		strcpy(buf + 7, "0;");
		checkMain = -1;	
	}
